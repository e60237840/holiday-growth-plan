...
create table if not exists public.tasks (
  ...
  star_awarded integer check (star_awarded between 0 and 999),
...
alter table public.tasks add column if not exists star_awarded integer check (star_awarded between 0 and 999);
...
begin
  ...
  update public.tasks set status = '已完成', completed_at = now(), approved_at = now(), reward_granted = true, star_awarded = v_star_amount where id = v_task.id returning * into v_task;
  return v_task;
end;
$$;
...
begin
  ...
  if v_requested_stars < v_task.star_reward and char_length(trim(coalesce(p_reason, ''))) < 2 then
  ...
  from public.point_transactions where user_id = v_user
    and created_at >= (public.family_today()::timestamp at time zone v_profile.timezone)
    and created_at < ((public.family_today() + 1)::timestamp at time zone v_profile.timezone);
  v_star_amount := least(v_requested_stars, greatest(v_profile.daily_points_cap - v_today_earned, 0));
  v_transaction_reason := case
    when v_requested_stars < v_task.star_reward then
      left(v_task.title || '（原定 ' || v_task.star_reward || ' 星，实际 ' || v_star_amount || ' 星：' || v_adjustment_note || '）', 200)
    else v_task.title
  end;
  v_profile.stars_balance := v_profile.stars_balance + v_star_amount;
  update public.profiles set stars_balance = v_profile.stars_balance where user_id = v_user;
  if v_star_amount > 0 then
    insert into public.point_transactions (user_id, task_id, amount, reason, transaction_type, operator, balance_after)
    values (v_user, v_task.id, v_star_amount, v_transaction_reason, '完成任务奖励', '家长', v_profile.stars_balance);
  end if;
  perform public.ensure_today_game_record();
  select * into v_record from public.game_time_records where user_id = v_user and record_date = public.family_today() for update;
  v_game_amount := least(v_task.game_minutes_reward, greatest(v_profile.max_game_minutes - v_record.base_minutes - v_record.earned_minutes, 0));
...
begin
  ...
  if p_kind = 'today' then
    update public.tasks set status = '未开始', completed_at = null, approved_at = null, reward_granted = false, star_awarded = null, penalty_applied = false, penalized_at = null, parent_note = null
...
revoke update (user_id, reward_granted, star_awarded, penalty_applied, completed_at, approved_at, penalized_at) on public.tasks from authenticated;
